...
received_message_index = np.load('received_message_index.npy') # Row index
FDI_index = np.load('attack_info.npy')
FDI_index_set = set(FDI_index.tolist())
rows=message_received.shape[0]
# Controllable three-phase load cabinet
...
# Line impedance simulation cabinet
...
for i in range(rows):
    print(i)
    k = received_message_index[i]  # Row index in the original message log
...
for i in range(len(data_line_cabinet_meter)):
    # Get the value from row i of the first column
...
# Save the time-aligned meter measurements to Excel files
