...
def resolve_log_file(configured_file):
    configured_path = Path(configured_file)
    if configured_path.exists():
        return configured_path

    candidates = sorted(Path(".").glob("log_*.log"))
    if len(candidates) != 1:
        raise FileNotFoundError(
            f"Configured log {configured_file!r} does not exist and "
            f"found {len(candidates)} log_*.log candidates"
        )
    return candidates[0]
