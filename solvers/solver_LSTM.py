...
class solver_LSTM(object):
    ...
    def __init__(self, config):
        ...
        elif self.LSTM_mode == "test":
            self.test_dataloader = get_loader_series(mode="test", **loader_args)
        else:
            raise ValueError(f"Unsupported LSTM mode: {self.LSTM_mode}")
    ...
    def test(self, return_details=False):
        ...
        test_file_name = os.path.basename(
            os.path.normpath(self.LSTM_testdata_path)
        ).lower()
        if test_file_name == "test_data_rd.xlsx":
            ...
            threshold_percentile = self.LSTM_threshold_percentile
