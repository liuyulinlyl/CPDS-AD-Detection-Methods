...
class solver_transformer(object):
    ...
    def __init__(self, config):
        ...
        if self.transformer_mode == "train":
        ...
        elif self.transformer_mode == "test":
            self.test_dataloader = get_loader_series(mode="test", **loader_args)
        else:
            raise ValueError(f"Unsupported Transformer mode: {self.transformer_mode}")
    ...
    def test(self, return_details=False):
        ...
        test_file_name = os.path.basename(
            os.path.normpath(self.transformer_testdata_path)
        ).lower()
        if test_file_name == "test_data_rd.xlsx":
            ...
            threshold_percentile = self.transformer_threshold_percentile
