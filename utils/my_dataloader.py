...
def _read_excel_cached(path, index_col=0):
    """Read each unchanged workbook once during a batch evaluation run."""
    key = (str(Path(path).resolve()), index_col)
    if key not in _EXCEL_CACHE:
        _EXCEL_CACHE[key] = pd.read_excel(path, index_col=index_col)
...
class my_dataloader_series:
    def __init__(self, win_size, step, mode="train", traindata_path='', testdata_path='', subseq_length=None):
        ...
        train_dataset = _read_excel_cached(traindata_path, index_col=0)
        ...
        test_dataset = _read_excel_cached(testdata_path, index_col=0)
